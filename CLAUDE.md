...
- **International phone input via intl-tel-input** (commit `59c4bfd`): customer phones now stored in **E.164 format** (`+60123456789`), entered through the `intl-tel-input` library (jackocnr, MIT, v23.0.7 from jsDelivr CDN). Country dropdown with flags + dial codes, autoformatting per-country, real validation. Default country Malaysia, neighbours pinned at top: SG/BN/ID/TH. Existing customers keep their legacy local format until next edit — the modal `setNumber()` parses and converts on save, so no backfill script needed. **Three helpers in shared.js** that every future phone-touching code path must use:
  ...
  - **Library cost**: ~30 KB CSS + ~80 KB JS (main + utils), CDN-cached after first hit. Negligible for the consistency win.
  - **Validation**: `iti.isValidNumber()` checked before save, blocks malformed numbers from reaching the DB. Empty input is allowed (phone is optional on customer records).
