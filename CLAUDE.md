...
Auto-notify a Telegram group on bookings + collections. Brainstormed → spec written then dropped per `feedback_skip_spec_doc` → built + deployed same session. Design doc at `docs/superpowers/specs/2026-06-13-oilpalm-telegram-notifications-design.md` (kept for reference). **Code is live but DORMANT until the user sets the two Netlify env vars** (see setup below) — until then the function returns `{ok:false,error:'not configured'}` and the frontend stays silent (no toast).
...
- **REMAINING USER SETUP (3 steps, then notifications go live)**: (1) @BotFather `/newbot` → copy token; (2) create a Telegram group, add the bot, send any message, open `https://api.telegram.org/bot<token>/getUpdates` → read `chat.id` (negative number for a group); (3) Netlify → Site settings → Environment variables → add `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID`, then redeploy (functions pick up env on next deploy). Final check after setup: `curl -X POST .../.netlify/functions/telegram-notify -d '{"kind":"message","text":"test"}'` → confirm it lands in the group.
